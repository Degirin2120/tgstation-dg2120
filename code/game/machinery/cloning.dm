...
/obj/machinery/clonepod/emag_act(mob/user)
	if(!occupant)
		return
	to_chat(user, "<span class='warning'>You bypass the ID lock, forcing an emergency ejection.</span>")
	go_out()
	add_fingerprint(user)
	log_cloning("[key_name(user)] emagged [src] at [AREACOORD(src)], ejecting the occupant.")
	log_combat(user, src, "emagged", null, occupant ? "[occupant] inside, ejecting them." : null)
